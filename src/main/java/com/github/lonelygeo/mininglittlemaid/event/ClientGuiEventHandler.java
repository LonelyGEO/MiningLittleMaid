...
public class ClientGuiEventHandler {
    ...
    public static void onInitMaidGui(MaidContainerGuiEvent.Init event) {
        ...
        int y = event.getTopPos() + 156;
        boolean[] enabled = {MaidMineBreakTask.isChatNotifyEnabled(maid)};
        Component value = Component.translatable(enabled[0] ? "options.on" : "options.off");
        MaidConfigButton button = new MaidConfigButton(x, y, CHAT_LABEL, value, btn -> {
            PacketDistributor.sendToServer(new MiningChatNotifyToggleMessage(maid.getId()));
            enabled[0] = !enabled[0];
            btn.setValue(Component.translatable(enabled[0] ? "options.on" : "options.off"));
        });
        event.addButton("mining_chat_notify", button);
    }
}
